...
function M.run(check, near)
  ...
  local r = run_mod.new(m, 2, "serpentine")
  r.money = 99999
  local pel = tower.place(r, 200, 150, "pellet")
  check(
    r.tower_stats[pel.id] and r.tower_stats[pel.id].damage == 0,
  ...
  check(
    rep.top_tower and rep.top_tower.kind == "pellet" and near(rep.top_tower.damage, 9),
    "report top-tower survives a sell"
  )

  -- splash credits both the direct target and every splash victim to the source
  local rs = run_mod.new(m, 3, "serpentine")
  rs.money = 99999
  ...
  check(near(rs.tower_stats[spl.id].damage, 12), "splash credits direct + splash applied to the source tower (6 + 6)")

  -- invulnerable boss hit credits nothing (boss.hurt returns 0 while phased out)
  local rb = run_mod.new(m, 4, "serpentine")
  wave.boss_scale(rb, 5)
  boss.spawn(rb, "prism", rb.hp_scale)
  rb.boss.invuln_t = 5
  ...
  local rc = run_mod.new(m, 12, "serpentine")
  rc.money = 99999
  ...
  local rf = run_mod.new(m, 13, "serpentine")
  rf.money = 99999
  ...
  local rlb = run_mod.new(m, 6, "serpentine")
  rlb.lives = 50
  wave.boss_scale(rlb, 5)
  boss.spawn(rlb, "prism", rlb.hp_scale)
  ...
  -- ---------------------------------------------------------- money spent
  local rm = run_mod.new(m, 7, "serpentine")
  rm.money = 99999
  ...
  local rd = run_mod.new(m, 16, "serpentine")
  rd.money = 99999
  rd.mods.cost_mult = 0.5
  local paid = tower.cost(rd, "pellet")
  tower.place(rd, 200, 150, "pellet")
  check(
    rd.money_spent == paid and paid < tower.DEFS["pellet"].cost,
  ...
  check(rep0.kills == 0 and rep0.leaked == 0 and rep0.money_spent == 0, "report: counters default to zero")
  ...
  check(report.build(r0).favorite_name == powerup.DEFS["dmg"].name, "report: favorite carries the powerup display name")
  ...
  check(tt and tt.kind == "splash" and near(tt.damage, 40), "report top-tower ranks the highest damage across towers")
