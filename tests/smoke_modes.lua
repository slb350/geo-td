...
function M.run(check, near)
  ...
  do
    local mr = meta.default()
    mr.unlocks.tower_rail = true
  ...
  do
    ...
    for i = 1, #boss.ROTATION do
      if boss.ROTATION[i] == kind then known = true end
    end
  ...
  do
    local rs = run_mod.new(m, 1, "serpentine")
    wave.boss_scale(rs, 5)
    local rh = run_mod.new(m, 1, "serpentine", "hardcore")
    wave.boss_scale(rh, 5)
  ...
  do
    ...
    local wisps = 0
    for i = 1, #fly do
      if fly[i].kind == "wisp" then wisps = wisps + 1 end
    end
  ...
  do
    ...
    for i = 1, #chaos do
      if chaos[i].rarity < 2 then all_boosted = false end
    end
  ...
  do
    check(
      report.build(run_mod.new(m, 1, "serpentine", "one_life")).mode == "One Life",
      "the run report carries the challenge-mode name"
    )
    check(report.build(run_mod.new(m, 1, "serpentine")).mode == nil, "a standard run report shows no mode")
  end
end
