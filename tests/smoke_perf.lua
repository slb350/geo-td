...
function M.run(check, near)
  ...
  check(
    type(result.max_step_time) == "number" and result.max_step_time >= 0,
    "perf: sim reports max per-step wall time"
  )
  check(step_budget > 0 and result.max_step_time <= step_budget, "perf: max sim step stays within budget")
  ...
  check(calls > 0 and draw_budget > 0 and calls <= draw_budget, "perf: draw-call count stays within harness budget")
  State.run = nil
end
