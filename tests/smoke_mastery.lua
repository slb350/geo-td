...
function M.run(check, near)
  ...
  local mp = meta.default(); mp.currency = 100
  ...
  check(near(base.mods.dmg_mult, 1) and base.mods.crit_chance == 0
    and near(base.mods.bounty_mult, 1) and base.mods.interest == 0,
  ...
  local function owned_run(id, mode_id)
    local mm = meta.default(); mm.mastery = { [id] = true }
  ...
  check(owned_run("survival_bulwark", "one_life").lives == 1,
    "One Life's fixed lives override ignores the Bulwark bonus")

  -- ---------------------------------------------------- v1 -> v2 save migration
  local v1 = {
    version = 1, currency = 50, best_wave = 9, total_runs = 4,
    unlocks = { tower_rail = true, tower_flak = false, start_bonus = false },
    map_unlocked = { zigzag = true, spiral = true }, map_best = { zigzag = 22 },
  }
  meta.save(v1)
  local up = meta.load()
  check(up.version == 2, "a v1 save migrates to v2")
  check(up.currency == 50 and up.best_wave == 9 and up.total_runs == 4,
    "migration preserves currency / best wave / run count")
  check(up.unlocks.tower_rail == true and up.map_best.zigzag == 22,
    "migration preserves unlocks + per-map bests")
  check(up.bank_shards == 0 and type(up.mastery) == "table" and type(up.badges) == "table"
    and type(up.contract_board) == "table" and type(up.completed_contracts) == "table"
    and type(up.daily) == "table", "migration backfills every v2 field")
  ...
  check(fixed.currency == 0 and fixed.best_wave == 0 and fixed.total_runs == 0,
  ...
  -- --------------------------------------------------------- menu Mastery panel
  State.meta = meta.default(); State.meta.currency = 100
