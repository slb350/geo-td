...
local function count_kind(r, kind)
  local n = 0
  for i = 1, r.enemies.n do if not r.enemies[i].dead and r.enemies[i].kind == kind then n = n + 1 end end
  return n
end

-- A fresh run with a freshly spawned boss of `kind` for boss wave n.
local function spawn(m, n, kind)
  local r = run_mod.new(m, n, "serpentine"); r.money = 99999
  wave.boss_scale(r, n); r.enemies.n = 0
  boss.spawn(r, kind, r.hp_scale)
  return r
end

local function kill_arena(r)
  for i = 1, r.enemies.n do if r.enemies[i].arena then enemy.damage(r, r.enemies[i], 1e9) end end
end
...
function M.run(check, near)
  ...
  local battery
  for i = 1, rb.enemies.n do if rb.enemies[i].arena then battery = rb.enemies[i]; break end end
  ...
  local ru = run_mod.new(m, 5, "serpentine"); ru.money = 99999
  ...
  ru.projectiles.n = 0; pel.cooldown = 0
  ...
  local bat
  for i = 1, rab.enemies.n do if rab.enemies[i].arena then bat = rab.enemies[i]; break end end
  local veil = enemy.spawn(rab, "veil", 1, 1, 0); veil.x, veil.y = bat.x, bat.y
  aura.update(rab)
  ...
  rr.arena_kills = 4; rr.final_boss_reached = true
