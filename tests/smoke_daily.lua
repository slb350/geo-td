...
function M.run(check, near)
  ...
  check(d1.map == d1b.map and d1.mode == d1b.mode and d1.seed == d1b.seed,
  ...
  check(mc.check(mc.DEFS.chicane_climb, { wave = 12, map = "Chicane" }),
    "a multi-condition goal needs all conditions")
  check(not mc.check(mc.DEFS.chicane_climb, { wave = 12, map = "Zigzag" }),
    "a wrong map fails the eq condition")
  ...
  -- -------------------------------------------------------- board refresh fill
  local m = meta.default(); mc.refresh(m)
  ...
  local m2 = meta.default(); m2.completed_contracts = { [mc.ORDER[1]] = true }
  mc.refresh(m2)
  check(#m2.contract_board == mc.BOARD_SIZE, "the board still fills to size around a completed goal")
  local skipped = true
  for i = 1, #m2.contract_board do if m2.contract_board[i] == mc.ORDER[1] then skipped = false end end
  check(skipped, "a completed goal is not re-offered on the board")

  -- ------------------------------------------------------------ board evaluate
  local me = meta.default(); mc.refresh(me)   -- board = first_blood, boss_trio, centurion
  local rep = { wave = 8, bosses_killed = 0, score = 0, contracts = 0,
                affixes = 0, arena_kills = 0, final_boss = false, map = "Serpentine" }
  local done = mc.evaluate(me, rep)
  local got_first = false
  for i = 1, #done do if done[i] == "first_blood" then got_first = true end end
  ...
  local left = true
  for i = 1, #me.contract_board do if me.contract_board[i] == "first_blood" then left = false end end
  check(left, "a completed goal leaves the board")

  -- --------------------------------------------------- gameover integration
  do
    State.meta = meta.default(); mc.refresh(State.meta)
  ...
  State.meta = meta.default(); mc.refresh(State.meta)
  State.ui.menu_tab = "contracts"; menu_s.draw(1 / 60)        -- the board renders without error
  State.ui.menu_tab = "daily"; menu_s.draw(1 / 60)
  ...
  State.run = nil; State.pending = nil; State.ui.menu_tab = "shop"
end
