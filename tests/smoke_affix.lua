...
local function count_kind(run, kind)
  ...
  for i = 1, run.enemies.n do
    if run.enemies[i].kind == kind then n = n + 1 end
  end
  return n
end
...
function M.run(check, near)
  ...
  local an
  for n = 8, 40 do
    if n % C.BOSS_EVERY ~= 0 and affix.for_wave(r, n) then
      an = n
      break
    end
  end
  ...
  local ra = run_mod.new(m, 1, "serpentine")
  ra.wave_affix = affix.DEFS.armored_front
  local armored = 0
  for idx = 1, 10 do
    ra.enemies.n = 0
  ...
  for i = 1, rmir.spawn_queue.n - 1, 2 do
    ...
    if not (a and b and a.fly ~= b.fly) then
      mirrored = false
      break
    end
  end
  ...
  -- ------------------------------------------------------ Overclock (speed)
  local ro = run_mod.new(m, 1, "serpentine")
  ro.wave_affix = affix.DEFS.overclock
  ...
  check(oe.affix_speed_mult ~= nil and near(oe.affix_speed_mult, 1.3), "Overclock applies a derived speed multiplier")
  ...
  local rv = run_mod.new(m, 1, "serpentine")
  rv.money = 99999
  rv.wave_affix = affix.DEFS.veiled_pack
  tower.place(rv, 200, 150, "pellet")
  rv.enemies.n = 0
  local source = enemy.spawn(rv, "veil", 1, 1, 0)
  source.x, source.y = 205, 150
  affix.apply_spawn(rv, source, 1, 10) -- vanguard -> veil source
  local cloaked = enemy.spawn(rv, "mote", 1, 1, 0)
  cloaked.x, cloaked.y = 210, 150
  affix.apply_spawn(rv, cloaked, 2, 10)
  affix.update(rv, 0)
  check(source.affix_veil_source == true and target.targetable(source), "Veiled Pack creates a targetable veil source")
  check(
    cloaked.affix_stealth == true and not target.targetable(cloaked),
    "the veil source protects nearby pack enemies"
  )
  rv.projectiles.n = 0
  rv.towers[1].cooldown = 0
  ...
  -- ----------------------------------------------------- Fracture (splits)
  local rf = run_mod.new(m, 1, "serpentine")
  rf.wave_affix = affix.DEFS.fracture
  rf.enemies.n = 0
  ...
  local child
  for i = 1, rf.enemies.n do
    if rf.enemies[i].kind == "mote" then child = rf.enemies[i] end
  end
  check(child and near(child.maxhp, enemy.DEFS.mote.hp * 1.6), "Fracture children are tougher")
  local rf0 = run_mod.new(m, 2, "serpentine")
  rf0.enemies.n = 0
  ...
  rn.money = 99999
  local t1 = tower.place(rn, 200, 150, "pellet")
  modifier.socket_module(rn, t1, "triangle")
  tower.place(rn, 215, 150, "pellet")
  modifier.socket_module(rn, rn.towers[2], "triangle")
  tower.place(rn, 200, 165, "pellet")
  modifier.socket_module(rn, rn.towers[3], "triangle")
  resonance.update(rn)
  check(t1.resonance ~= nil, "Triad resonance forms before Null Field")
  rn.wave_affix = affix.DEFS.null_field
  rn.affix_null_active = true
  rn.affix_timer = 10
  resonance.update(rn)
