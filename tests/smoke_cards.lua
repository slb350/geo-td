...
function M.run(check, near)
  ...
  do
    ...
    local t = tower.place(r, 200, 150, "pellet")
    r.enemies.n = 0
    dummy(r, 205, 150) -- the only enemy
    t.cooldown = 0
    tower.update(r, 1 / 60)
    settle(r)
  ...
  do
    ...
    local fly = enemy.spawn(r, "wisp", 20, 1, 5)
    fly.x, fly.y = 214, 150
  ...
  do
    ...
    for _ = 1, C.RING_TICKS * math.ceil(C.RING_INTERVAL * 60) + 10 do
      ring.update(r, 1 / 60)
    end
  ...
  do
    local r = fresh(18)
    r.enemies.n = 0
    local sp = enemy.spawn(r, "splitter", 1, 1, 10)
    sp.x, sp.y = 100, 100
  ...
  do
    local r = fresh(19)
    r.mods.brittle = 1.0 -- +100% to slowed
    r.enemies.n = 0
    local e = dummy(r, 100, 100)
    enemy.slow(e, 0.5, 5)
  ...
  do
    local r = fresh(9)
    r.mods.flyer_burst = 0.5
    r.enemies.n = 0
    local a = enemy.spawn(r, "wisp", 1, 1, 5)
    a.x, a.y = 100, 100
    local b = enemy.spawn(r, "wisp", 1, 1, 5)
    b.x, b.y = 110, 100 -- within C.FLYER_BURST_R
    local b_hp0 = b.hp
    enemy.kill(r, a)
