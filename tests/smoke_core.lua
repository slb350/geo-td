-- Broad baseline cases for the smoke suite: balance constants, meta/save, run
-- setup, every path layout, map-unlock progression, placement, splitter/shield/
-- regen/flyer mechanics, rail/flak targeting, powerups, the non-rail boss
-- fixtures, the 1-20 wave simulation, the orbital strike, and the scene layer.
-- Split out of tests/smoke.lua (now a thin runner) so each suite stays well under
...
require("tests.harness") -- installs the fake engine globals (side effect only)
