...
function M.run(check, near)
  ...
  r.wave_index = 2; check(contracts.pending(r), "contract pending before wave 3 (cadence)")
  r.wave_index = 3; check(not contracts.pending(r), "no contract before wave 4 (off cadence)")
  r.wave_index = 14; check(not contracts.pending(r), "no contract before a boss wave (15)")

  -- --------------------------------------------------------------- the draft
  local rd = run_mod.new(m, 7, "serpentine"); rd.wave_index = 2   -- next wave = 3
  local d3 = contracts.draft(rd)
  check(#d3 >= 1 and #d3 <= 3, "draft at wave 3 offers cards (capped at 3)")
  local has_blackout = false
  for _, c in ipairs(d3) do if c.id == "blackout" then has_blackout = true end end
  ...
  local same = (#a == #b)
  for i = 1, #a do if a[i].id ~= b[i].id then same = false end end
  check(same, "contract draft is deterministic for seed/wave")

  -- --------------------------------------------------------- sign / decline
  local rs = run_mod.new(m, 1, "serpentine"); rs.wave_index = 5
  ...
  local rdec = run_mod.new(m, 1, "serpentine"); contracts.decline(rdec)
  check(rdec.active_contract == nil, "decline leaves no active contract")

  -- ------------------------------------------------- arm: blackout + no-sell
  local rb = run_mod.new(m, 1, "serpentine"); rb.money = 99999; rb.wave_index = 5
  ...
  local rn = run_mod.new(m, 1, "serpentine"); rn.money = 99999; rn.wave_index = 5
  ...
  local nt = tower.place(rn, 200, 150, "pellet"); local kept = #rn.towers
  tower.sell(rn, nt)
  check(#rn.towers == kept, "no-sell blocks selling a tower")

  -- ----------------------------------------------- wave.start risk channels
  local rh0 = run_mod.new(m, 1, "serpentine"); wave.start(rh0, 3)
  local rh = run_mod.new(m, 1, "serpentine"); rh.active_contract = { risk = { hp_mult = 1.5 } }
  wave.start(rh, 3)
  check(near(rh.hp_scale, rh0.hp_scale * 1.5), "Enrage spikes the wave's hp_scale")
  local rc0 = run_mod.new(m, 2, "serpentine"); local base_n = wave.start(rc0, 3)
  local rc = run_mod.new(m, 2, "serpentine"); rc.active_contract = { risk = { count_mult = 2.0 } }
  ...
  rr.active_contract = { reward = { charge = 18 } }; contracts.grant_reward(rr)
  check(rr.charge == 18, "grant_reward pays charge")
  rr.active_contract = { reward = { money = 60 } }; local mb = rr.money; contracts.grant_reward(rr)
  check(rr.money == mb + 60, "grant_reward pays money")
  rr.active_contract = { reward = { bank_shard = 1 } }; contracts.grant_reward(rr)
  check(rr.bank_shards == 1, "grant_reward pays bank shards")
  rr.active_contract = { reward = { module_discount = 30 } }; contracts.grant_reward(rr)
  check(rr.module_discount == 30, "grant_reward grants a module discount")
  rr.no_sell = true; contracts.expire(rr)
  check(rr.active_contract == nil and rr.no_sell == false, "expire clears the contract + no-sell flag")

  -- ----------------------------------------------------- the module discount
  local rmd = run_mod.new(m, 1, "serpentine"); rmd.money = 99999
  ...
  rrep.contract_history = { "enrage", "swarm" }; rrep.bank_shards = 2
  ...
  -- --------------------------------------------------------- contract scene
  do
    State.run = run_mod.new(State.meta, 7, "serpentine"); State.run.wave_index = 5
    State.pending = nil
    ...
    input.mouse = function() return x0 + TW * 0.5, 62 + 40 end
    contract_s.update(1 / 60)
    input.mouse_pressed, input.mouse_held, input.mouse = real_mp, real_mh, real_mo
    check(State.run.active_contract ~= nil and State.pending == "game",
  ...
  do
    ...
    State.run.phase = "combat"; State.run.wave_index = 3
    State.run.active_contract = { id = "enrage", risk = {}, reward = { charge = 18 } }
    State.run.charge = 0; State.run.lives = 0           -- already fatally leaked
