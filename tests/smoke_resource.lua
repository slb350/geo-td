...
function M.run(check, near)
  ...
  check(
    r2.resource_nodes[1].x == r.resource_nodes[1].x and r2.resource_nodes[1].y == r.resource_nodes[1].y,
  ...
  -- --------------------------------------------------------- Drill placement
  local rp = run_mod.new(m, 4242, "serpentine")
  rp.money = 99999
  ...
  local rd = run_mod.new(m, 4242, "serpentine")
  rd.money = 99999
  ...
  local rbk = run_mod.new(m, 5, "serpentine")
  rbk.money = 99999
  ...
  rdis.charge = 50
  rdis.enemies.n = 0
  local e1 = enemy.spawn(rdis, "hulk", 1, 1, 10)
  ...
  check(reroll_cost > 0, "module reroll has a charge cost")
  check(
    type(modifier.can_reroll_module) == "function" and modifier.can_reroll_module(rr, rt),
    "a socketed tower with enough charge can reroll its module"
  )
  ...
  check(
    type(modifier.can_reroll_module) == "function" and not modifier.can_reroll_module(rr, rt),
    "module reroll rejects insufficient charge"
  )
  local no_mod = tower.place(rr, 210, 70, "pellet")
  rr.charge = reroll_cost
  check(
    type(modifier.can_reroll_module) == "function" and not modifier.can_reroll_module(rr, no_mod),
    "module reroll requires an existing module"
  )
