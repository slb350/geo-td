...
print(("baseline | standard | waves 1-%d | uniform pellet cordon step %d | seed %d")
  :format(WAVES, STEP, SEED))
print(("%-11s %4s %4s %5s %5s %5s %7s %6s  %s")
  :format("map", "wave", "surv", "leak", "pkEn", "pkPj", "spend", "rfund", "top damage"))
...
for _, name in ipairs(maps.ORDER) do
  ...
  print(("%-11s %4d %4s %5d %5d %5d %7d %6d  %s"):format(
    name, r.final_wave, r.survived and "yes" or "NO", r.leaks,
    r.peak_enemies, r.peak_proj, r.money_spent, r.route_refunds,
    top and (top.kind .. " " .. math.floor(top.damage)) or "-"))

  -- boss seconds-to-clear, in wave order
  local bw = {}
  for w in pairs(r.boss_seconds) do bw[#bw + 1] = w end
  table.sort(bw)
  local parts = {}
  for i = 1, #bw do
